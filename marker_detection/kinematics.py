...
class VisualServoingNode(Node):
    ...
    def check_marker_timeout(self):
        ...
        if time.time() - self.last_aruco_time > 5.0:
    ...
    def aruco_callback(self, msg):
        ...
        # 매번 초기화하여 현재 위치 기준으로 스무딩 (Jumping 방지)
        self.prev_q1_target = None
        self.prev_q2_target = None
        ...
        z_cam = msg.position.z * 1000.0 - 10.0 # 실제 거리보다 1cm(10mm) 길게 나오므로 보정
        ...
        # [수정] 거리 오차에 따른 급격한 L2_virtual 변경이 Link 2의 튐 현상(Jumping)을 유발함.
        # 따라서 조건부 변경 로직을 제거하고 고정된 Virtual Link 길이를 사용하거나,
        # 부드럽게 보간해야 함. 여기서는 일단 고정 길이로 복구하여 안정성 확보.
